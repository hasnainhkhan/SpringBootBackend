...
public class BookController {
	...
	try {
	    ...
	    return ResponseEntity.status(HttpStatus.CONFLICT).body(null); // ResponseEntity for error with no body (or
									  // you can add error message inside a custom
									  // object)
	} catch (Exception e) {
	    e.printStackTrace();
	...
	try {
	    ...
	    return ResponseEntity.status(HttpStatus.CONFLICT).body(null); // Or you can return a custom error object
	} catch (Exception e) {
	    e.printStackTrace();
	    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
	}
